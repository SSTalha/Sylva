        ...
        while i < len(self.tokens):
            ...
            if token_type == 'DATA_TYPE':
                ...
                if self.tokens[i + 2][1] == 'ASSIGNMENT':
                    ...
                    self.assembly_code.append(f"LOAD {value_token[2]}")
                ...
                if value == 'if':
                    ...
                    self.assembly_code.append(f"LOAD {condition[2]}")
                    ...
                    self.label_count += 1
                    end_label = f"L{self.label_count}"
                    self.assembly_code.append(f"LOAD {init[2]}")
                    self.assembly_code.append(f"{start_label}:")
                    self.assembly_code.append(f"LOAD {condition[2]}")
                    self.assembly_code.append(f"JZ {end_label}")
                    i += self.handle_block(i + 6)
                    self.assembly_code.append(f"LOAD {increment[2]}")
                    self.assembly_code.append(f"JMP {start_label}")
                    self.assembly_code.append(f"{end_label}:")
