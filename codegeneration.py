        ...
        while i < len(self.tokens):
            ...
            if token.type == 'DATA_TYPE':
                ...
                if self.tokens[i + 2].type == 'ASSIGNMENT':
                    ...
                    self.assembly_code.append(f"LOAD {value_token.value}")
                    self.assembly_code.append(f"STORE {variable_name}")
                i += 4
                ...
                if self.is_function_call(i):
                    ...
                    for arg in arguments:
                        self.assembly_code.append(f"LOAD {arg}")
                    self.assembly_code.append(f"CALL {function_name}")
                else:
                    # Handle variable usage
                    variable_name = token.value
                    self.assembly_code.append(f"LOAD {variable_name}")
                i += 1
                ...
                if token.value == 'if':
                    ...
                    self.assembly_code.append(f"LOAD {condition.value}")
                    self.assembly_code.append(f"JZ {label}")
                    ...
                    self.assembly_code.append(f"LOAD {condition.value}")
                    self.assembly_code.append(f"JZ {end_label}")
                    i += self.handle_block(i + 2)
                    ...
                    self.label_count += 1
                    end_label = f"L{self.label_count}"
                    self.assembly_code.append(f"LOAD {init.value}")
                    self.assembly_code.append(f"{start_label}:")
                    self.assembly_code.append(f"LOAD {condition.value}")
                    self.assembly_code.append(f"JZ {end_label}")
                    i += self.handle_block(i + 6)
                    self.assembly_code.append(f"LOAD {increment.value}")
                    self.assembly_code.append(f"JMP {start_label}")
                    self.assembly_code.append(f"{end_label}:")
